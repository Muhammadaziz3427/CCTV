...
## Opening the App Storage pane

You can render a one-click "Open in appStorage" button that takes the user to the App Storage workspace pane — uploaded objects, buckets, or user-generated files. To render it, write this tag on its own line at the end of your reply (it is replaced by the button; the user clicks it to open the pane):

```
<open-in-pane tool="appStorage"></open-in-pane>
```
...
| --- | --- |
| "Where are my uploaded files?" | `<open-in-pane tool="appStorage"></open-in-pane>` |
| "Open App Storage / show the bucket." | `<open-in-pane tool="appStorage"></open-in-pane>` |
...
Call `setupObjectStorage()` in the `codeExecution` sandbox to provision the bucket. This is **idempotent** — if the bucket already exists it returns immediately with `alreadySetUp: true`.
...
### Step 6: Required Upload Protection

The provided upload URL route rejects unauthenticated requests by default. Set up Replit Auth before enabling user uploads:

1. Set up Replit Auth first (see the `replit-auth` skill)
2. Mount `authMiddleware` before the storage routes
3. Keep `req.isAuthenticated()` guarding the upload endpoint
4. Use `req.user.id` as the owner when setting ACL policies

When file uploading is requested, both **Replit Auth** and **PostgreSQL** must also be configured, even if not explicitly mentioned. Persistent storage and user authentication are implicitly required for user file uploads. Public object serving through `/storage/public-objects/*` is separate and does not require auth.
...
- Keep the upload URL route protected by `req.isAuthenticated()`
- Use ACL policies to control access (see `objectAcl.ts`)

When file uploading is requested, both **Replit Auth** and **PostgreSQL** must also be configured, even if not explicitly mentioned. Persistent storage and user authentication are implicitly required for user file uploads.
