...
- `logout()` navigates to `/api/logout?returnTo=<BASE_URL>` (full page redirect, preserves artifact base path)
...
| `/logout` | GET | Clears session, redirects to OIDC end-session endpoint. Accepts optional `?returnTo=` query param for the post-logout redirect (validated, defaults to `/`) |
