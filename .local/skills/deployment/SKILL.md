...
1. To update deployment settings, use `verifyAndReplaceArtifactToml` from the artifacts skill (temp-file workflow).
2. After verifying the app works, call `SuggestUserAction({ action: "deploy", message: "The app is ready to publish." })`.
